...
class Bot:
    ...
    def plan(self) -> list[str]:
        ...
        # 훈련한도는 HQ레벨에만 달려있고 L2는 한도를 안 올려준다(1->1) --
        # L3에서 처음 1->2로 오른다 (game-rule.md §TRAIN). 무조건 조기화는
        # 회귀였다(경제/물량을 갉아먹음) -- 이미 물량에서 밀리고 있을 때만
        # (반응형) 훈련한도 병목을 뚫는 캐치업으로 한정한다.
        if (getattr(self, "unknown_map", False) and t >= 70
                and len(self.enemy) >= len(self.mine) + 8):
            hq_target = max(hq_target, min(hq_cap, 3))
        # anti-greed HQ 게이트 v2 (평가#25 AlphaBeta·평가#26 freeconferencefood/
        # 스카이 3연속 실측): 상대가 HQ L1 고정으로 순수 확장/물량에 올인하면
        # 격차가 T75부터 T135+까지 계속 벌어진다 (turn<=80 한정이던 구버전은
        # 진짜 위기가 시작되기도 전에 꺼졌음). 한번 켜지면 상대가 HQ를 올리거나
        # 우리가 따라잡을 때까지 유지한다 (래치, 플래핑 방지).
        their_hq_lv1 = (their_hq_b is not None and their_hq_b[0] == self.foe
                        and their_hq_b[2] <= 1)
        self._antigreed_latch = getattr(self, "_antigreed_latch", False)
        if (getattr(self, "unknown_map", False) and 60 <= t <= 150
                and invaders < 3 and their_hq_lv1
        ...
        # HQ L1 서킷브레이커 (평가#26 올크크전 실측): near_enemy>=1 하나로도
        # under_pressure가 걸려서 저축 예약(elif)이 게임 내내 한 번도
        # 실행 안 될 수 있다 -- 즉시구매만으로는 훈련이 여유 골드를 계속
        # 먹어치워 600골드가 안 모인다. T80 넘도록 여전히 L1이면 위협
        # 여부와 무관하게 HQ를 최우선 저축 대상으로 강제한다. (하)에서
        # 훈련 hold 계산에도 쓰이므로 이 블록 밖에서 항상 계산해둔다.
        ...
        if (saving_for == 0 or _hq_bypass) and hq is not None and hq[2] < hq_target:
            ...
            _extra = 300 if (_hq_bypass and not under_pressure and not _hq_stuck) else 0
            if ((not delay_last) and can_upgrade_region(self.my_hq)
                    and gold >= cost + reserve + _extra):
                do_upgrade(self.my_hq, cost)
            elif (saving_for == 0
                  and (not (self.attacking or under_pressure) or hq_rush_now
                       or hq_race or _hq_stuck)
        ...
        # 상대가 기지를 하나씩 순차로 각개격파할 때(평가#26 GPT-5.6전 실측:
        # 우리 기지 5->0, T70~80 30턴 만에 전멸) 단일 목표만 방어하면 항상
        # 한 발 늦는다 -- 위협받는 곳이 2개 이상이면 두 번째로 위험한 곳에도
        # 소수를 나눠 보낸다.
        defend_at = None
        defend_at2 = None
        ...
        if (getattr(self, "unknown_map", False) and self.turn >= 60
            ...
            if _en_bases:
                ...
                for w in sorted(free, key=lambda w: self.dist[self.mine[w][0]][_ht])[:5]:
                    ...
                    move_to(w, _ht)
                free = [w for w in free if w not in assigned]
                # 실전 최상위권(GPT-5.6) 리버스엔지니어링: 병력이 아주
                # 여유로울 때만(3배) 2번째 목표에 소수(2기)만 얹는다 --
                # 더 크게(3기, 2배 문턱) 시도했더니 econ에서 소폭 회귀.
        ...
        if hq is not None:
            ...
            if ((hq_rush_now or hq_race or _hq_stuck) and saving_for > 0
