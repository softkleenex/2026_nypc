...
class Bot:
    ...
    def plan(self) -> list[str]:
        ...
        if self._desp_latch:
            def race_route_steps(src: int, target: int, blocked: set[int]) -> int:
                steps = 0
                cur = src
                charged: set[int] = set()
                while cur != target and steps <= self.N * 2:
                    if cur in blocked and cur not in charged:
                        steps += 1
                        charged.add(cur)
                    best_v = -1
                    best_score = math.inf
            ...
            def race_steps(pool: dict[str, list], target: int,
                ...
                best = 10**9
                blocked = {x[0] for x in blockers.values()}
                for s in pool.values():
                    if s[2]:
                        first = s[3]
                        steps = race_route_steps(s[0], first, blocked)
                        if steps < 10**9 and first != target:
                            steps += race_route_steps(first, target, blocked)
                    else:
                        steps = race_route_steps(s[0], target, blocked)
                    best = min(best, steps)
                return best
