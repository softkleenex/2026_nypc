...
OFFICIAL_MAP_PROFILES: dict[str, dict[str, object]] = {
    ...
    "5fc20ca6f1d9": {
        ...
        "params": {
            ...
            "first_attack_start_no_base": 150,
            "first_attack_start_with_base": 150,
    ...
    "efcea1183068": {
        ...
        "params": {
            ...
            "economy_boom_recall_hop": 6,
            "base_defense_hop": 5,
            "base_defense_enemy_trigger": 1,
            "base_defense_margin": 2,
            "base_defense_max": 12,
            "urgent_defense_hop": 5,
...
def decide(S: GameState, M: GameMap, P: Paths, turn: int) -> Actions:
    ...
    if hq is not None:
        ...
        if adaptive_economy_clear and base_count > 0:
        ...
        if enemy_alive >= alive + M.params["target_army_enemy_margin"] and not (economy_hq2 or stale_hq2 or late_pressure_hq2):
            saving_for_hq = False
        late_hq_train_buffer = TRAIN_COST * max(1, train_cap) if profile_tuned else 0
        saving_for_late_hq = late_hq_upgrade_needed and reserved_gold < late_hq_next_cost + late_hq_train_buffer
        if urgent_training_needed:
            saving_for_late_hq = False
    ...
    for b in S.buildings:
        ...
        keep = base_worker_home_keep if b.region == M.my_hq else (b.work_cap() if base_worker_fill_enabled else 1)
    ...
    staging_mode = not M.params["disable_staging"] and turtle_like and stage != M.my_hq
    ...
    adaptive_expansion_guard = adaptive_economy_clear and base_count < desired_bases and turn < 90
    ordered_idle = sorted(
        idle,
