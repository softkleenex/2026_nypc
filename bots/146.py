...
class Bot:
    ...
    def plan(self) -> list[str]:
        ...
        early_hq_guard = (getattr(self, "unknown_map", False)
                          and 45 <= t < 100
                          and hq is not None and hq[2] == 1
                          and n_bases <= 3
                          ...
                          and invaders >= 3)
        if early_hq_guard:
            hq_target = max(hq_target, min(hq_cap, 2))
        ...
        if self._antigreed_latch and not (_hq_stuck or need_train_cap or early_hq_guard):
            hq_target = 1
        ...
        if allin_alert or early_hq_guard:
            ...
            if can_upgrade_region(self.my_hq) and gold >= cost + reserve + _extra:
                do_upgrade(self.my_hq, cost)
                  ...
                  and (not (self.attacking or under_pressure)
                       or hq_race or hq_score_race or _hq_stuck
                       or need_train_cap or early_hq_guard)
        ...
        expansion_ok = (not early_hq_guard and not threatened
        ...
        if hq is not None:
            ...
            if (early_hq_guard and saving_for > 0 and not behind):
                hold = saving_for
            elif ((hq_race or hq_score_race or _hq_stuck or need_train_cap) and saving_for > 0
