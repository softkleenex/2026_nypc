...
class Bot:
    ...
    def plan(self) -> list[str]:
        ...
        endgame_chip = False
        if (self.mp.get("chip") and self.turn >= 165 and my_hq_b is not None
                and their_hq_b is not None and their_hq_b[0] == self.foe):
            if their_hq_b[3] > my_hq_b[3]:
                endgame_chip = True
            elif their_hq_b[3] == my_hq_b[3]:
                endgame_chip = len(self.mine) >= len(self.enemy) + 2
        ...
        if (self.mp.get("chip") and self.turn >= 165 and my_hq_b is not None
            ...
            endgame_chip = (
                ...
                or (their_hq_b[3] == my_hq_score_hp
                    and len(self.mine) >= len(self.enemy) + 2)
            )
        ...
        if self.turn >= 182 or endgame_chip:
