...
def valid_data_payload(data: object) -> bool:
    ...
    return (
        ...
        and (source is None or source in {"21.py", os.path.basename(__file__)})
    )
...
def decide(S: GameState, M: GameMap, P: Paths, turn: int) -> Actions:
    ...
    if max_new_bases > 0:
        ...
        pending_guard_keep = max(1, int(M.params["pending_build_guard_keep"]))
