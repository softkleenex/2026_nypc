...
class Bot:
    ...
    def plan(self) -> list[str]:
        ...
        if (getattr(self, "unknown_map", False) and self.turn == 100
                and invaders < 3 and len(self.enemy) > len(self.mine)):
        ...
        # (참가자전 미지맵에 L3/12기지 탐욕을 이식하는 A/B는 econ 5승->0승으로
        # 기각됨: 탐욕이 승리 재원을 희석한다. 고티어 매크로는 플래그로 복제 불가.)
        ...
        if saving_for == 0 and hq is not None and hq[2] < hq_target:
            ...
            if (not delay_last) and can_upgrade_region(self.my_hq) and gold >= cost + reserve:
                do_upgrade(self.my_hq, cost)
            elif ((not (self.attacking or under_pressure) or hq_rush_now or hq_race)
        ...
        if getattr(self, "unknown_map", False):
            ...
            threatened = near1 >= 3 or near_enemy >= 5 or invaded or rush_alert
        else:
        ...
        if (getattr(self, "unknown_map", False)
                ...
                and (not threatened or invaders <= 4) and not self.attacking
                and n_bases < 5 and invaders <= 4 and self.turn >= 30):
            contested = [
                r for r in self.strongholds
