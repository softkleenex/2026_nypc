...
class Bot:
    ...
    def plan(self) -> list[str]:
        ...
        def enemy_pressure_at(origin: int, k: int) -> int:
            ...
            project_pressure = self.turn >= 140 and n_en_bases <= n_my_bases
            for wid, s in self.enemy.items():
                h = self.hops[origin][s[0]]
                projected = False
                step = recent_enemy_step.get(wid)
                if project_pressure and step is not None:
                    old_r, new_r = step
                    projected = (
        ...
        desperate_hq_catchup = (
            getattr(self, "unknown_map", False) and self.turn >= 150
            and desperate and invaders < 3
        ...
        if desperate_hq_catchup and n_bases >= 3:
            expandable = False  # 카운터 중이어도 턴제한 HP 열세면 추가 확장보다 HQ 캐치업이 우선이다.
        ...
        if (saving_for == 0 or _hq_bypass) and hq is not None and hq[2] < hq_target:
            ...
            if can_upgrade_region(self.my_hq) and gold >= cost + reserve + _extra:
                do_upgrade(self.my_hq, cost)
            elif (_hq_bypass and (hq_race or _hq_stuck or need_train_cap)
        ...
        if saving_for == 0 and not hq_upgrade_blocked and not desperate_hq_catchup:
