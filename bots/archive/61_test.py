...
class Bot:
    ...
    def read_init(self) -> None:
        ...
        self.unknown_map = True  # [일회성 테스트 전용] 강제 True --
        # 8개 공식 샘플봇 상대로 unknown_map 로직(관문게이트·견제부대 등)이
        # 실제 독립코딩 상대에게 통하는지 검증하기 위함. MAP_PROFILES(맵별
        # 하드코딩)는 여전히 적용되므로 프로필과 신규 로직이 동시에 켜진다.
        # 대표 답안으로 쓰지 말 것 -- 재현보호가 이 테스트에선 의도적으로 깨짐.
        if self.unknown_map:
