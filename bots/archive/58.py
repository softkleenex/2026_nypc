...
class Bot:
    ...
    def _sim_campaign(self, my_hps, en_hps, my_train_hp, en_train_hp,
                      my_cap, en_rate, en_turret, en_bldg_hp, horizon):
        """향후 horizon턴 동안 매일 증원하며 상호 소모전을 예측 (룰 §7).
        상대 총병력을 수비 풀로 가정(비관적/보수적), 상대 미래 훈련은
        최근 관측 평균으로 외삽. 지평선 내 건물 파괴 시 True (원정 타당)."""
        mine = list(my_hps); enemy = list(en_hps); bldg = en_bldg_hp
        carry = 0.0
        for _ in range(horizon):
            mine += [my_train_hp] * my_cap
            carry += en_rate
            while carry >= 1.0:
                enemy.append(en_train_hp)
                carry -= 1.0
            if not mine:
                return False
            if bldg <= 0:
                return True
            mine.sort(); enemy.sort()
            my_atk = len(mine)
            en_atk = len(enemy) + (en_turret if bldg > 0 else 0)
            for _ in range(my_atk):
                if enemy and enemy[0] > 0:
                    enemy[0] -= 1
                    if enemy[0] == 0: enemy.pop(0)
    ...
    def plan(self) -> list[str]:
        ...
        # 다턴 공성 캠페인 예측 (미지맵, 110턴+): 단일 턴 스냅샷이 아니라
        # 향후 25턴의 상호 증원+소모전을 시뮬레이션해 실제 돌파 가능성을
        # 본다 (56의 실패 원인 = 한 턴 교환만 봄. 이번엔 여러 날에 걸친
        # 캠페인 -- 관측된 적 훈련률로 외삽, 우리 총병력 vs 적 총병력 가정).
        campaign_ready = False
        if (getattr(self, "unknown_map", False) and self.turn >= 110
                and not self.attacking
                and my_hq_b is not None and their_hq_b is not None
                and their_hq_b[0] == self.foe):
            _hist = getattr(self, "_en_train_hist", [])
            if len(_hist) >= 20:
                _window = _hist[-30:]
                _en_rate = sum(_window) / len(_window)
                _my_lv, _en_lv = my_hq_b[2], their_hq_b[2]
                campaign_ready = self._sim_campaign(
                    [w[1] for w in self.mine.values()],
                    [w[1] for w in self.enemy.values()],
                    HQ_LV[_my_lv][1], HQ_LV[_en_lv][1], HQ_LV[_my_lv][4],
                    _en_rate, HQ_LV[_en_lv][3], their_hq_b[3], 25)
        if ((army_size >= p["push_army"] and hq_lv >= p["hq_push_lv"] and advantage)
                ...
                or endgame_chip or desperate or big_idle or campaign_ready):
            self.attacking = True
    ...
    def read_result(self) -> None:
        ...
        if n > 0:
            for wid in readln().split():
                ...
                if side != self.side:
                    _en_spawned += 1
        self._en_train_hist = getattr(self, "_en_train_hist", [])
        self._en_train_hist.append(_en_spawned)
