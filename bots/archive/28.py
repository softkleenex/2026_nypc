...
PARAMS = {
    ...
    "target_army_base_late_base": 10,
    "target_army_per_base": 5,
    ...
    "score_hq_min_army": 18,
    "score_hq_min_bases": 0,
    "score_hq_train_buffer": 0,
    "score_hq_upgrade_max_level": 5,
    "late_hq_reserve_turn": 120,
    "late_hq_reserve_min_army": 14,
    "late_hq_reserve_max_level": 5,
    "late_hq_worker_floor": 4,
    "late_hq_lock_turn": 155,
    "anti_idle_train_turn": 45,
    "anti_idle_enemy_margin": 4,
    "late_swarm_train_turn": 145,
    "late_swarm_enemy_count": 45,
    "late_swarm_train_cap": 120,
    "late_swarm_stage_hop": 2,
    "late_swarm_commit_cap": 70,
    "late_swarm_home_keep": 4,
    "overdefense_release_gap": 2,
    "overdefense_release_hop": 3,
    "overdefense_release_keep": 3,
    "near_hq_commit_turn": 999,
    "near_hq_commit_hop": 2,
    "near_hq_commit_edge": 1,
    "near_hq_commit_min_near": 7,
    "base_defense_abort_enemy": 8,
    "base_defense_abort_edge": 2,
...
def decide(S: GameState, M: GameMap, P: Paths, turn: int) -> Actions:
    ...
    enemy_region_counts = warrior_counts_by_region(enemies)
    enemy_biggest_stack = max(enemy_region_counts.values(), default=0)
    ...
    immediate_home_pressure = sum(
        1 for w in holding_enemies
        if P.hop[w.region][M.my_hq] <= M.params["overdefense_release_hop"]
    )
    release_overdefense = (
        urgent_home_threat
        and not any(w.region == M.my_hq for w in enemies)
        and economy_gap >= M.params["overdefense_release_gap"]
        and raw_enemy_home_hop > 1
    )
    if release_overdefense:
        home_garrison_target = min(
            home_garrison_target,
            max(
                M.params["overdefense_release_keep"],
                immediate_home_pressure + M.params["urgent_defense_enemy_margin"],
            ),
        )
    ...
    score_hq_upgrade_needed = (
        ...
        and hq.level < M.params["score_hq_upgrade_max_level"]
    ...
    late_swarm_active = (
        turn >= M.params["late_swarm_train_turn"]
        and base_count >= 5
        and enemy_alive >= M.params["late_swarm_enemy_count"]
        and not any(w.region == M.my_hq for w in enemies)
    )
    late_hq_reserve_active = (
        hq is not None
        and hq.level < hq.max_level()
        and hq.level < M.params["late_hq_reserve_max_level"]
    ...
    late_swarm_front_safe = immediate_home_pressure == 0 and raw_enemy_home_hop > 3
    if late_swarm_active and late_swarm_front_safe:
        late_hq_reserve_active = False
    late_hq_lockdown = (
        hq is not None
        and hq.level < hq.max_level()
        and turn >= M.params["late_hq_lock_turn"]
        and (enemy_hq_level > hq.level or late_hq_reserve_active)
        and not (late_swarm_active and late_swarm_front_safe)
    )
    hq_missing_hp = (
        hq.current_hp() - hq.hp
        if hq is not None and hq.level >= hq.max_level()
    ...
    worker_backed_base_upgrades = 0
    worker_backed_base_upgrade_total = sum(
        1 for b in S.buildings
        if b.side is M.my_side and b.type is BType.BASE and b.level >= 2
    )
    ...
    for r in M.strongholds:
        if r in upgrade_regions or not can_upgrade_here(S, M, r):
            continue
        b = S.find_building(r)
        worker_backed_base_upgrade = False
        if b is None:
            ...
            assigned_workers = sum(
                1 for w in mine
                if w.region == r or (w.state is WState.MOVING and w.target == r)
            )
            worker_backed_base_upgrade = (
                b.type is BType.BASE
                and turn >= M.params["worker_backed_base_upgrade_turn"]
                and b.level < M.params["worker_backed_base_upgrade_max_level"]
                and assigned_workers >= M.params["worker_backed_base_upgrade_worker_min"]
                and raw_enemy_home_hop > M.params["worker_backed_base_upgrade_home_hop"]
                and worker_backed_base_upgrades < M.params["worker_backed_base_upgrade_max_per_turn"]
                and worker_backed_base_upgrade_total < M.params["worker_backed_base_upgrade_max_total"]
                and not any(w.region == M.my_hq for w in enemies)
            )
            ...
            cost = b.upgrade_cost()
            gold_buffer = (
                M.params["worker_backed_base_upgrade_gold_buffer"]
                if worker_backed_base_upgrade
                else M.params["base_upgrade_gold_buffer"]
            )
            if (
                not worker_backed_base_upgrade
                and (
                    len(mine) < M.params["base_upgrade_min_army"]
                    or reserved_gold < cost + gold_buffer
                )
            ):
                continue
            if (
                worker_backed_base_upgrade
                and reserved_gold < cost + gold_buffer
            ):
                continue
        else:
            continue
        ...
        if (
            late_hq_reserve_active
            and not allow_hq2_economy_base
            and not worker_backed_base_upgrade
            and reserved_gold < late_hq_next_cost + cost
        ):
            continue
        ...
        if reserved_gold >= cost:
            ...
            elif worker_backed_base_upgrade:
                worker_backed_base_upgrades += 1
                worker_backed_base_upgrade_total += 1
    ...
    if hq is not None:
        ...
        if (
            ...
            target_army = max(
                target_army,
                min(M.params["late_swarm_train_cap"], enemy_alive + M.params["target_army_enemy_margin"]),
            )
        ...
        training_build_cutoff = M.params["build_cutoff_turn"]
        if base_count == 0 or losing_economy:
            training_build_cutoff = M.params["emergency_build_cutoff_turn"]
        training_build_cutoff = max(training_build_cutoff, policy_value(policy, 5, 0))
        saving_for_base = (
            turn < training_build_cutoff
            and base_count < desired_bases
            and reserved_gold < base_gold_reserve
        )
        ...
        production_gap = (
            turn >= M.params["anti_idle_train_turn"]
            and hq.level >= 2
            and enemy_alive >= alive + M.params["anti_idle_enemy_margin"]
            and not any(w.region == M.my_hq for w in enemies)
            and raw_enemy_home_hop > 1
        )
        if production_gap and reserved_gold >= TRAIN_COST:
            saving_for_late_hq = False
            if not hq_heal_emergency:
    ...
    if (
        ...
        for b in my_bases:
            ...
            local_reinforcements = nearby_warrior_count(
                [
                    w for w in mine
                ...
                P,
                b.region,
                M.params["base_defense_hop"] + (1 if economy_alert else 0),
            )
            if (
                len(nearby_enemies) >= M.params["base_defense_abort_enemy"]
                and assigned + local_reinforcements < len(nearby_enemies) + M.params["base_defense_abort_edge"]
                and not any(w.region == M.my_hq for w in enemies)
            ):
                continue
            desired = min(
    ...
    swarm_forward_orders: dict[WarriorId, int] = {}
    if (
        late_swarm_active
        and not any(w.region == M.my_hq for w in enemies)
        and (late_swarm_front_safe or enemy_biggest_stack >= M.params["late_swarm_enemy_count"])
        and (
            enemy_opp_hq_count >= M.params["late_swarm_enemy_count"]
            or enemy_biggest_stack >= M.params["late_swarm_enemy_count"]
        )
    ):
        choke = home_choke_region(M, P, int(M.params["late_swarm_stage_hop"]))
        if choke != M.my_hq:
            assigned = sum(
                1 for w in mine
                if w.region == choke or (w.state is WState.MOVING and w.target == choke)
            )
            home_keep = max(home_garrison_target, int(M.params["late_swarm_home_keep"]))
            desired = min(
                int(M.params["late_swarm_commit_cap"]),
                max(12, max(enemy_opp_hq_count, enemy_biggest_stack) - home_keep),
            )
            need = max(0, desired - assigned)
            selected_home_swarm = 0
            candidates = [
                w for w in idle
                if w.region != choke
                and w.id not in builder_orders
            ...
            for w in candidates:
                if len(swarm_forward_orders) >= need:
                    break
                if w.region == M.my_hq and home_count_for_build - selected_home_swarm <= home_keep:
                    continue
                swarm_forward_orders[w.id] = choke
                if w.region == M.my_hq:
                    selected_home_swarm += 1
        ...
        if choke != M.my_hq and raw_enemy_home_hop > stage_hop:
            ...
            def releasable_for_forward(worker: Warrior) -> bool:
                if worker.id in builder_orders or worker.id in base_defender_orders or worker.id in home_defender_orders:
                    return False
                if worker.id in swarm_forward_orders:
                    return False
    ...
    near_hq_commit_hop = int(M.params["near_hq_commit_hop"])
    near_hq_friend_count = sum(1 for w in mine if P.hop[w.region][M.opp_hq] <= near_hq_commit_hop)
    near_hq_enemy_count = sum(1 for w in enemies if P.hop[w.region][M.opp_hq] <= near_hq_commit_hop)
    near_hq_commit_active = (
        turn >= M.params["near_hq_commit_turn"]
        and near_hq_friend_count >= M.params["near_hq_commit_min_near"]
        and near_hq_friend_count >= near_hq_enemy_count + building_turret(enemy_hq) + M.params["near_hq_commit_edge"]
        and not urgent_home_threat
        and not mass_home_threat
        and not any(w.region == M.my_hq for w in enemies)
    )
    ...
    for w in ordered_idle:
        ...
        if w.id in home_intercept_orders:
        ...
        elif w.id in swarm_forward_orders:
            target = swarm_forward_orders[w.id]
            ...
            and P.hop[w.region][M.opp_hq] <= near_hq_commit_hop
        ):
            target = M.opp_hq
        ...
        if (
            w.moved_last_turn
            and w.prev_region == target
            and target not in (M.my_hq, M.opp_hq)
            and w.id not in home_intercept_orders
            and w.id not in builder_orders
            and w.id not in base_worker_orders
            and (hq_worker_order is None or w.id != hq_worker_order)
