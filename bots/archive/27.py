...
PARAMS = {
    ...
    "home_fortify_turn": 999,
    "home_fortify_hop": 2,
    "home_fortify_max_level": 3,
    "home_fortify_gold_buffer": 120,
    "home_fortify_min_army": 12,
...
def home_fortify_region(S: GameState, M: GameMap, P: Paths) -> int | None:
    max_hop = int(M.params["home_fortify_hop"])
    candidates = [
        b.region for b in S.buildings
        if b.side is M.my_side
        and b.type is BType.BASE
        and b.level < min(b.max_level(), int(M.params["home_fortify_max_level"]))
        and P.hop[M.my_hq][b.region] <= max_hop
    ]
    if not candidates:
        return None
    choke = home_choke_region(M, P, max_hop)
    return min(
        candidates,
        key=lambda r: (
            P.hop[r][choke],
            -P.hop[M.my_hq][r],
            P.dist[r][M.opp_hq],
            r if M.my_side is Side.LEFT else -r,
        ),
    )
...
def decide(S: GameState, M: GameMap, P: Paths, turn: int) -> Actions:
    ...
    fortify_region = (
        home_fortify_region(S, M, P)
        if (
            turn >= M.params["home_fortify_turn"]
            and len(mine) >= M.params["home_fortify_min_army"]
            and not urgent_home_threat
            and not any(w.region == M.my_hq for w in enemies)
        )
        else None
    ...
    if (
        fortify_region is not None
        and fortify_region not in upgrade_regions
        and can_upgrade_here(S, M, fortify_region)
    ):
        cost = upgrade_cost_for(S, fortify_region, M.my_side)
        if reserved_gold >= cost + M.params["home_fortify_gold_buffer"]:
            a.upgrades.append(fortify_region)
            upgrade_regions.add(fortify_region)
            reserved_gold -= cost
