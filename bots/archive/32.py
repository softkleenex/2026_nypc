...
class Bot:
    ...
    def plan(self) -> list[str]:
        ...
        # HQ 레이스 예외 (서버 6/7/8 패인 = 전부 레벨 티어브레이크): 상시
        # 유격형이 압박을 유지하면 전시경제가 테크를 영구 동결시킨다. 110턴
        # 이후 상대 HQ 레벨이 내 레벨 이상이고 대규모 침공이 아니면, 압박
        # 중이어도 HQ 저축을 허용한다 -- 턴제한 판정은 결국 본부 싸움이다.
        hq_race = (
            self.turn >= 110 and invaders < 8
        ...
        if saving_for == 0 and hq is not None and hq[2] < hq_target:
            ...
            if can_upgrade_region(self.my_hq) and gold >= cost + reserve:
                do_upgrade(self.my_hq, cost)
            elif ((not (self.attacking or under_pressure) or hq_race)
        ...
        for r, b in self.my_buildings():
            ...
            # 기지 보초 (서버 8번 패인): 유격 2명이 노동자 1명짜리 기지를
            # 순회 철거한다. 적이 4홉 이내인 기지는 한도+1명을 상주시켜
            # 보초+노동자+포탑으로 소규모 유격을 자체 격퇴한다.
            hold_n = cap
            if (r != self.my_hq and len(self.mine) >= 8 and self.turn >= 25
                    and enemy_within(r, 4)):
                hold_n = cap + 1
            here = [w for w in self.my_at(r) if w in movable and w not in assigned]
            for w in here[:hold_n]:
                assigned.add(w)
            worker_slots += cap
            heading_here = sum(1 for s in self.mine.values() if s[2] and s[3] == r)
            lack = hold_n - len(here[:hold_n]) - heading_here
            if lack > 0:
        ...
        # 호위 확장 (서버 7번 교착 해소): 유격 몇 명이 모든 확장 거점을
        # 베토해서 기지 2개로 200턴을 보내는 것을 막는다. 클레임 가능한
        # 거점이 없고 유격도 소수면, 가장 가까운 경합 거점을 집단으로
        # 청소하러 간다 -- 우세가 확보되면 claimable이 열려 클레임이 재개된다.
        if (defend_at is None and not threatened and not self.attacking
                and my_bases < 4 and invaders <= 4 and self.turn >= 25):
            contested = [
                r for r in self.strongholds
        ...
        free = [w for w in movable if w not in assigned]
        # 집단 요격 (서버 8번 패인): 침입자에게 한 명씩 걸어가면 1대3 헌납의
        # 연속이다 (야전 전투 피해 49). 응전 병력이 침입자보다 많을 때만 나간다.
        if defend_at is not None and invaders >= 1 and len(free) < invaders + 1:
            defend_at = None  # 머릿수가 모일 때까지 본진/집결지에서 대기
        ...
        if hq is not None:
            ...
            if hq_race and saving_for > 0:
                hold = saving_for  # HQ 레이스 저축은 훈련으로부터 보호한다
