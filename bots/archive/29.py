...
class Bot:
    ...
    def plan(self) -> list[str]:
        ...
        # 침공 감지: 내 반경에 들어온 적 병력이 임계 이상이면 전면 방어 태세.
        # 상대 푸시는 웨이브 단위로 오므로, 집중 스택 + 본부 포탑으로 각개 요격한다.
        invaders = sum(
            1 for s in self.enemy.values()
            if self.dist[self.my_hq][s[0]] < self.dist[self.opp_hq][s[0]]
        )
        invaded = invaders >= 8    # 침공(웨이브 규모): 소개 + 본부 농성
        raided = invaders >= 3     # 소규모 습격: 병력으로 요격 (경제는 유지)
        # 초반 러시 감지: 개막 직후 적 전사가 내 쪽 절반 가까이 들어오면
        # (샘플/공격형 봇의 3전사 러시) 확장을 멈추고 집을 지킨다. 이때 나가
        # 있으면 본부 1:3 싸움 -> 훈련병 축차 소모 -> 경제 0 교착으로 진다.
