...
OFFICIAL_MAP_PROFILES: dict[str, dict[str, object]] = {
    ...
    "fee7bfd1e85e": {
        ...
        "params": {
            ...
            "force_hq2_turn": 75,
            "force_hq2_min_army": 7,
            "score_hq_upgrade_turn": 88,
            "late_hq_reserve_turn": 88,
            "late_hq_reserve_min_army": 7,
            "late_hq_worker_floor": 5,
...
def nearest_open_strongholds(S: GameState, M: GameMap, P: Paths) -> list[int]:
    ...
    for r in M.strongholds:
        ...
        state_score += 240 * enemy_near1 + 110 * enemy_near2
        state_score -= 45 * my_near
        state_score -= 18 * len(M.adj[r])
        state_score -= 45 * mirror_enemy_base
        state_score += 450 * overextended
        score = base_score + max(-15, min(15, state_score))
...
def decide(S: GameState, M: GameMap, P: Paths, turn: int) -> Actions:
    ...
    urgent_training_needed = urgent_home_threat and len(mine) < urgent_army_target
    hq2_gold_candidate = (
        hq is not None
        and hq.level == 1
    ...
    if (late_hq_reserve_active and hq is not None and hq.level >= 3 and base_count > 0) or hq_heal_reserve_active:
        economy_mode = False
    ...
    home_under_worker_threat = enemy_home_hop <= M.params["base_worker_threat_hop"] or any(w.region == M.my_hq for w in enemies)
    ...
    base_worker_home_keep = max(
        ...
        M.params["late_hq_worker_floor"] if (late_hq_reserve_active or late_hq_lockdown or hq_heal_reserve_active) else 0,
    )
    for b in S.buildings:
    ...
    staging_mode = not M.params["disable_staging"] and turtle_like and stage != M.my_hq and not late_hq_lockdown and not hq_heal_reserve_active
    ...
    adaptive_expansion_guard = base_count < desired_bases and turn < M.params["build_cutoff_turn"]
    ...
    for w in ordered_idle:
        ...
        if force_hq2 and cost > 0 and reserved_gold < HQ_LEVELS[2].upgrade_cost:
            continue
        if late_hq_reserve_active and cost > 0 and reserved_gold < late_hq_next_cost + cost:
            continue
