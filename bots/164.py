...
class Bot:
    ...
    def base_intent_counts(self) -> dict[int, int]:
        if not getattr(self, "unknown_map", False):
            return {}
        counts: dict[int, int] = {}
        for wid, (cands, steps, last_turn, _hot_steps) in self.enemy_intent.items():
            if wid not in self.enemy or self.turn - last_turn > 1:
                continue
            if steps < 2 or len(cands) > 2:
                continue
            targets = [
                r for r in cands
                if (b := self.buildings.get(r)) is not None
                and b[0] == self.side and not b[1]
            ]
            if len(targets) == 1:
                counts[targets[0]] = counts.get(targets[0], 0) + 1
        return counts
    ...
    def plan(self) -> list[str]:
        ...
        # 이동 의도 기반 선제 기지 방어: 후보 교집합이 작은 적 이동이 내
        # 비HQ 기지를 정확히 가리키면, 도착 전에 소수만 먼저 붙인다.
        if (getattr(self, "unknown_map", False) and defend_at is None
                and not threatened and not self.attacking and not invaded):
            base_intents = self.base_intent_counts()
            base_intents = {r: n for r, n in base_intents.items() if n >= 3}
            if base_intents:
                _n, _r = max(
                    base_intents.items(),
                    key=lambda kv: (kv[1], -self.dist[self.my_hq][kv[0]]),
                )
                already = sum(
                    1 for s in self.mine.values()
                    if s[0] == _r or (s[2] and s[3] == _r)
                )
                need = max(0, min(3, _n + 1) - already)
                for w in sorted(free, key=lambda w: self.dist[self.mine[w][0]][_r])[:need]:
                    move_to(w, _r)
                if need > 0:
        ...
        for _ in range(n):
            ...
            if s is not None:
                ...
                if wid[0] == self.foe and prev_r != r:
                    ...
                    edge_hot = (prev_r, r) in self.intent_hot_edges or (
