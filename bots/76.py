...
class Bot:
    ...
    def read_init(self) -> None:
        ...
        self.unknown_map = True  # 검증 완료(61 제출): 알려진 8맵도 범용로직이 프로필보다 강함(8승0패, 60=7승1무)
        if self.unknown_map:
    ...
    def plan(self) -> list[str]:
        ...
        # 적 소유 건물(본부/기지)에 정착해 일하는 워커는 침공군이 아니다.
        # 순수 거리 기준만 쓰면 midline 너머로 정당하게 확장한 적 기지의
        # 워커까지 "침공"으로 오분류해 확장동결/워커소개 등 방어 태세를
        # 허위로 발동시킨다 (건물에 서 있지 않은, 미확보 지역 이동 중인
        # 유닛만 침공군으로 센다).
        invaders = sum(
            1 for s in self.enemy.values()
            if self.dist[self.my_hq][s[0]] < self.dist[self.opp_hq][s[0]]
            and self.buildings.get(s[0], (None,))[0] != self.foe
        )
        ...
        # anti-greed HQ 게이트 (미지맵 전용, 평가#25 AlphaBeta전 패인 진단):
        # 상대가 HQ 투자를 아예 안 하면서(L1 고정) 기지/물량에서 이미 앞서고
        # 있으면, 우리도 HQ 저축을 접고 훈련/확장에 전념한다 (T47 600골드
        # 테크가 T54 4기 기지습격 방어를 못 하게 만든 게 직접 원인이었음).
        # 상대가 따라잡히거나(우리가 앞서거나) HQ를 올리면 다음 턴 바로 해제.
        if (getattr(self, "unknown_map", False) and 35 <= t <= 80
                and their_hq_b is not None and their_hq_b[0] == self.foe
                and their_hq_b[2] <= 1 and invaders < 3
        ...
        if (saving_for == 0 or _hq_bypass) and hq is not None and hq[2] < hq_target:
            ...
            # 지속압박에 서서히 갈리는 함정이 생긴다 (평가#25 실측 진단).
            _extra = 300 if (_hq_bypass and not under_pressure) else 0
        ...
        if saving_for == 0:
            for r, b in sorted(self.my_buildings(), key=lambda rb: self.dist[self.my_hq][rb[0]]):
                if b[1] or not can_upgrade_region(r):
                    continue
                # 만렙(3) 기지는 더 올릴 수 없고, 손상돼도 이 루프가 건너뛰기만
                # 해서 지금까지 절대 수리되지 않았다 (BASE_REPAIR_COST가
                # 정의만 되고 어디서도 쓰이지 않는 죽은 상수였던 게 그 증거).
                # 레벨3 구역에 UPGRADE를 내면 500골드 수리로 만피 회복이다.
        ...
        # 기회주의 강습 (미지맵): 상대가 본진을 비운 순간(본부 2홉 이내 수비
        # 1기 이하)엔 정상 물량 문턱을 못 채웠어도 나간다 -- 지금까지는
        # "우리가 준비됐는가"만 봤지 "상대가 비었는가"는 한 번도 안 봤다.
        opportunist = (getattr(self, "unknown_map", False)
                       and self.turn >= 50 and not threatened
                       and force_near(self.enemy, self.opp_hq, 2) <= 1
                       and army_size >= max(p["wave"], int(p["push_army"] * 0.6)))
        # 강제출격 문턱을 "언제(_pt+25)"가 아니라 "웨이브가 언제부터 준비돼
        # 있었나"로 판단한다. 전투 손실로 문턱 도달이 늦어지면(과거: 도달
        # 즉시 미준비 강제출격) 실제 준비 시점부터 25턴을 다시 센다. 일찍
        # 준비되면 max(_pt, 준비시점)=_pt라 기존과 동일한 _pt+25에 발동한다.
        if army_size >= p["wave"]:
            if getattr(self, "_wave_ready_turn", None) is None:
                self._wave_ready_turn = self.turn
        else:
            self._wave_ready_turn = None
        if ((army_size >= p["push_army"] and hq_lv >= p["hq_push_lv"] and advantage)
                or (self.turn >= _pt and advantage)
                or (self._wave_ready_turn is not None
                    and self.turn >= max(_pt, self._wave_ready_turn) + 25
                    and army_size >= p["wave"])
                or (self.mp.get("raid") and 110 <= self.turn < 175
                    and invaders < 3 and army_size >= p["wave"])
                or endgame_chip or desperate or big_idle or opportunist):
            self.attacking = True
        ...
        # 상대가 기지를 하나씩 순차로 각개격파할 때(평가#25 갈치전 진단) 단일
        # 목표만 방어하면 항상 한 발 늦는다 -- 위협받는 곳이 2개 이상이면
        # 두 번째로 위험한 곳에도 소수를 나눠 보낸다.
        defend_at = None
        defend_at2 = None
        if (raided or (near_enemy >= 1 and not self.mp.get("gen29"))) and not invaded:
            ...
            if cands:
                if getattr(self, "unknown_map", False):
                    cands.sort(key=lambda c: (c[0], -self.dist[self.my_hq][c[1]]),
                               reverse=True)
                else:
                    cands.sort(reverse=True)
                defend_at = cands[0][1]
                if getattr(self, "unknown_map", False) and len(cands) > 1:
                    defend_at2 = cands[1][1]
        ...
        for w in free:
            ...
            if ring_pressure and _ring_def < _ring_need:
                ...
                if defend_at2 is not None and not _sent_to_defend2:
                    move_to(w, defend_at2)
                    _sent_to_defend2 = True
                else:
                    move_to(w, defend_at)
                ...
                # 수비(터렛+상시 훈련)에 각개격파당한다 (실측: turtle 상대
                # 무승부 게임에서 T150 이후 40+턴간 아군 피해 84건, 공성 0 --
                # "시뮬 게이트 구현했다"던 이전 보고가 실제로는 저장 안 된
                # 기능이었음을 오늘 감사로 발견). 웨이브가 찰 때까지 관문에서
                # 뭉치고, 종반에는 무조건 진입(턴제한 안전판).
                _gate_n = sum(1 for x in self.mine.values()
                             if x[0] in self.adj[self.opp_hq])
                if _gate_n >= 20 or self.turn >= 195:
