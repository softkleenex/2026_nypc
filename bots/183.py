...
DATA_FORMATS = {"nypc-156-intent-json", "nypc-158-intent-combined-json"}
_DATA_CACHE = None
_DATA_LOADED = False
...
class Bot:
    def __init__(self, style: str) -> None:
        ...
        self.gold = 500
        self.enemy_intent: dict[str, tuple[frozenset[int], int, int, int]] = {}
        self._intent_cands: dict[tuple[int, int], frozenset[int]] = {}
        self.intent_hot_edges: set[tuple[int, int]] = set()
        self.intent_policy: set[str] = set()
        self.target_policy: set[str] = set()
        self.group_hq_policy: set[str] = set()
        self._last_group_hq_turn = -999
        self._last_group_trace = 0
        self._last_group_target = 0
    ...
    def map_fingerprint(self) -> str:
        lines = [
            ...
            " ".join(map(str, self.x)),
            " ".join(map(str, self.y)),
            "STRONGHOLDS " + " ".join(map(str, self.strongholds)),
        ]
        lines.extend(f"{len(row)} " + " ".join(map(str, row)) for row in self.adj)
        return hashlib.sha1("\n".join(lines).encode()).hexdigest()[:12]

    def data_bin_paths(self) -> list[str]:
    ...
    def load_data(self) -> dict:
        ...
        for path in self.data_bin_paths():
            try:
            ...
            try:
                ...
                continue
            if isinstance(data, dict) and data.get("format") in DATA_FORMATS:
                _DATA_CACHE = data
                _DATA_LOADED = True
                return data
        _DATA_CACHE = {}
    ...
    def load_intent_data(self) -> None:
        data = self.load_data()
        policy = data.get("policy") if isinstance(data, dict) else None
        if isinstance(policy, dict):
            self.intent_policy = set(policy)
        target_policy = data.get("target_policy") if isinstance(data, dict) else None
        if isinstance(target_policy, dict):
            self.target_policy = {
                key for key, value in target_policy.items()
                if isinstance(value, list) and len(value) >= 2 and value[1] == "my_hq"
            }
        group_hq_policy = data.get("group_hq_policy") if isinstance(data, dict) else None
        if isinstance(group_hq_policy, dict):
            self.group_hq_policy = set(group_hq_policy)
        profiles = data.get("profiles") if isinstance(data, dict) else None
        profile = profiles.get(self.map_fingerprint()) if isinstance(profiles, dict) else None
        if not isinstance(profile, dict):
            return
        side_key = "right" if self.flip else "left"
    ...
    def group_hq_policy_key(self, trace_support: int, target_support: int) -> str | None:
        if trace_support <= 0 and target_support <= 0:
            return None
        vals = (
            self._bucket(trace_support, (0, 1, 2, 3, 6, 10)),
            self._bucket(target_support, (0, 1, 3, 6, 10)),
        )
        return ",".join(str(v) for v in vals)

    def intent_policy_key(self, src: int, dst: int, cands: frozenset[int]) -> str | None:
        if self.my_hq not in cands:
            return None
        src_home = self.hops[src][self.my_hq]
        dst_home = self.hops[dst][self.my_hq]
        dst_opp = self.hops[dst][self.opp_hq]
        advance = src_home - dst_home
        if advance < 0:
            return None
        cand_b = self._bucket(len(cands), (1, 3, 5, 8, 12, 20))
        home_b = self._bucket(dst_home, (1, 2, 3, 4, 6, 9))
        rel_b = max(-4, min(4, dst_home - dst_opp))
        turn_b = self._bucket(self.turn, (20, 40, 70, 110, 150))
        return ",".join(str(v) for v in (cand_b, home_b, rel_b, min(3, advance), turn_b))

    def target_policy_key(
        self,
        src: int,
        dst: int,
        cands: frozenset[int],
        ...
        src_home = self.hops[src][self.my_hq]
        dst_home = self.hops[dst][self.my_hq]
        src_opp = self.hops[src][self.opp_hq]
        dst_opp = self.hops[dst][self.opp_hq]
        if min(src_home, dst_home, src_opp, dst_opp) >= 10**8:
            return None
        vals = (
            self._bucket(len(cands), (1, 3, 5, 8, 12, 20)),
            self._bucket(len(merged), (1, 3, 5, 8, 12, 20)),
            min(4, steps),
            1 if self.my_hq in cands else 0,
            1 if any((b := self.buildings.get(r)) is not None and b[0] == self.side for r in cands) else 0,
            1 if self.opp_hq in cands else 0,
            1 if any((b := self.buildings.get(r)) is not None and b[0] == self.foe for r in cands) else 0,
            1 if any(r in self.strongholds for r in cands) else 0,
            1 if self.my_hq in merged else 0,
            1 if any((b := self.buildings.get(r)) is not None and b[0] == self.side for r in merged) else 0,
            self._bucket(dst_home, (1, 2, 3, 4, 6, 9)),
            self._bucket(dst_opp, (1, 2, 3, 4, 6, 9)),
            max(-4, min(4, dst_home - dst_opp)),
            max(-4, min(4, src_home - dst_home)),
            max(-4, min(4, src_opp - dst_opp)),
            self._bucket(self.turn, (20, 40, 70, 110, 150)),
        )
        return ",".join(str(v) for v in vals)
    ...
    def next_step(self, src: int, target: int) -> int:
        if src == target:
            return -1
        best_v = -1
        best_score = math.inf
        ...
        for v in self.adj[src]:
            score = math.ceil(math.hypot(self.x[src] - self.x[v],
                                         self.y[src] - self.y[v])) + self.dist[target][v]
            tie = -v if self.flip else v
    ...
    def intent_candidates(self, src: int, observed_next: int) -> frozenset[int]:
        key = (src, observed_next)
        cands = self._intent_cands.get(key)
        if cands is None:
            cands = frozenset(
                target for target in range(self.N)
                if target != src and self.next_step(src, target) == observed_next
            )
            self._intent_cands[key] = cands
        return cands

    def hq_intent_count(self) -> int:
        if not getattr(self, "unknown_map", False):
            return 0
        count = 0
        for wid, (cands, steps, last_turn, hot_steps) in self.enemy_intent.items():
            if wid not in self.enemy or self.turn - last_turn > 1:
                continue
            tight_route = steps >= 2 and len(cands) <= 5 and self.my_hq in cands
            data_route = hot_steps >= 2 and steps >= 2 and len(cands) <= 12 and self.my_hq in cands
            if tight_route or data_route:
                count += 1
        if self.group_hq_policy and self.turn - self._last_group_hq_turn <= 1:
            key = self.group_hq_policy_key(self._last_group_trace, self._last_group_target)
            if key is not None and key in self.group_hq_policy:
                count = max(count, min(6, max(3, self._last_group_trace, self._last_group_target)))
        return count
    ...
    def plan(self) -> list[str]:
        ...
        if self._desp_latch:
            my_race = min((self.route_steps(s[0], self.opp_hq) for s in self.mine.values()),
                          default=10**9)
            enemy_race = min((self.route_steps(s[0], self.my_hq) for s in self.enemy.values()),
                             default=10**9)
        ...
        early_fortress = (getattr(self, "unknown_map", False)
                          and self.turn < 70
                          and len(self.enemy) >= len(self.mine) + 1
                          and invaders >= 3
                          and near_enemy >= 3)
        incoming_hq = self.hq_intent_count()
        # 초반 러시 감지: 개막 직후 적 전사가 내 쪽 절반 가까이 들어오면
        # 확장을 멈추고 집을 지킨다.
        their_hq_early = self.buildings.get(self.opp_hq)
        delayed_mass_base = (
            getattr(self, "unknown_map", False)
            and 25 <= self.turn < 60
            and their_hq_early is not None and their_hq_early[0] == self.foe
            and their_hq_early[2] <= 1
            and len(self.enemy) >= len(self.mine) + 3
        )
        if delayed_mass_base and n_en_bases < n_my_bases and self.turn < 50:
            self._delayed_mass_seen = True
        delayed_mass_alert = (
            delayed_mass_base
            and (
                (self.turn < 35 and n_en_bases < n_my_bases
                 and len(self.enemy) >= len(self.mine) + 4)
                or
                (getattr(self, "_delayed_mass_seen", False) and invaders >= 2)
                or (n_en_bases <= n_my_bases and invaders >= 4)
            )
        )
        ...
        if delayed_mass_alert:
            rush_alert = True
        intent_data_ready = bool(self.intent_hot_edges)
        intent_pressure_min = 3 if intent_data_ready else 4
        intent_threat_min = 4 if intent_data_ready else 5
        if incoming_hq >= intent_pressure_min and self.turn < 80:
            rush_alert = True
        # 올인 러시 감지:
        ...
        under_pressure = raided or near_enemy >= 1 or outmassed or (
            incoming_hq >= intent_pressure_min and self.turn < 120)
        ...
        macro_catchup_expand = (
            getattr(self, "unknown_map", False)
            and self.flip
            and 55 <= t < 115
            and n_en_bases >= n_my_bases + 3
            and len(self.enemy) >= len(self.mine) + 2
            and enemy_pressure_near(3) == 0
            and not allin_alert
        )
        if macro_catchup_expand:
            base_target = max(base_target, min(12, len(self.strongholds)))
        hq_cap = p["hq_cap"]
        ...
        early_hq_cap_risk = (
            ...
            and hq is not None and hq[2] == 1
            and n_bases >= 4
            and n_en_bases >= n_my_bases + 2
            and len(self.enemy) >= len(self.mine) + 3
            and enemy_pressure_near(1) == 0
            and not allin_alert
            and (
                incoming_hq >= intent_pressure_min
                or invaders >= 2
                or near_enemy >= 1
                or n_en_bases >= n_my_bases + 3
            )
        )
        need_train_cap = (
            getattr(self, "unknown_map", False)
            and (
                (t >= 70 and len(self.enemy) >= len(self.mine) + 8)
                or early_hq_cap_risk
            )
        )
        if need_train_cap:
        ...
        early_greed = (40 <= t < 60 and n_en_bases >= n_my_bases + 2)
        ...
        if allin_alert or delayed_mass_alert:
        ...
        if n_bases < base_target and expandable:
            ...
            for r in build_order:
                ...
                if self.my_at(r) and not self.enemy_count_at(r):
                    _collapse_rebuild = (
                        getattr(self, "unknown_map", False)
                        and t >= 45 and n_bases <= 1
                        and force_near(self.enemy, r, 2) == 0
                    )
                    if gold >= BASE_LV[1][0] + (0 if _collapse_rebuild else reserve):
        ...
        hq_score_race = (
            getattr(self, "unknown_map", False) and self.turn >= 120
            and not desperate and enemy_pressure_near(1) == 0
            and my_hq_b is not None and their_hq_b is not None
            and their_hq_b[0] == self.foe and their_hq_b[2] > my_hq_b[2]
        )
        ...
        if (saving_for == 0 or _hq_bypass) and hq is not None and hq[2] < hq_target:
            ...
            _extra = 300 if (_hq_bypass and not under_pressure
                             and not _hq_stuck and not (hq_race or hq_score_race)) else 0
            if can_upgrade_region(self.my_hq) and gold >= cost + reserve + _extra:
                do_upgrade(self.my_hq, cost)
            elif (saving_for == 0
                  and (not (self.attacking or under_pressure)
                       or hq_race or hq_score_race or _hq_stuck or need_train_cap)
        ...
        hq_upgrade_blocked = (
            ...
            and not (hq_race or hq_score_race or _hq_stuck or need_train_cap)
        )
        ...
        for r, b in self.my_buildings():
            ...
            keep_cap = cap
            if (macro_catchup_expand and r != self.my_hq and released_settlers < 2
                    and n_bases < base_target and not enemy_within(r, 2)):
                keep_cap = max(0, cap - 1)
                released_settlers += cap - keep_cap
            here = [w for w in self.my_at(r) if w in movable and w not in assigned]
            for w in here[:keep_cap]:
                assigned.add(w)
            ...
            worker_slots += min(keep_cap, len(here[:keep_cap]) + heading_here)
            lack = keep_cap - len(here[:keep_cap]) - heading_here
            if lack > 0:
        ...
        if getattr(self, "unknown_map", False):
            ...
            threatened = (
                near1 >= 3 or invaded or rush_alert or early_fortress
                or (incoming_hq >= intent_threat_min and self.turn < 120)
            )
            ring_pressure = (near_enemy >= 5 and not threatened)
        else:
            threatened = near_enemy >= p["threat_min"] or invaded or rush_alert
        if desperate or collapse_finish:
        ...
        normal_attack = (
            ...
            or endgame_chip or desperate or collapse_finish or big_idle
        )
        ...
        elif getattr(self, "wave_target", None) in enemy_bldgs:
        ...
        if (getattr(self, "unknown_map", False) and self.turn >= 60
                ...
                and not desperate and not collapse_finish
        ...
        for w in free:
            ...
            if ring_pressure and _ring_def < _ring_need:
                ...
                if _gate_n >= 35 or self.turn >= 195 or collapse_finish:
        ...
        if hq is not None:
            ...
            if ((hq_race or hq_score_race or _hq_stuck or need_train_cap) and saving_for > 0
                    and not behind and (not threatened or early_hq_cap_risk)):
                hold = saving_for
            ...
            if self.turn >= 160:
                ...
                if (
                    incoming_hq >= intent_threat_min
                    and enemy_pressure_near(3) >= incoming_hq
                ):
                    supply_fund = 0
            ...
            if (getattr(self, "unknown_map", False) and afford < 1 and hold > 0
                    ...
                    and not (self.flip and (
                        hq_score_race
                        or (self.turn >= 160 and hq_race
                            and n_bases >= n_en_bases + 4
                            and len(self.mine) >= len(self.enemy))
                    ))):
                afford = 1
    ...
    def read_result(self) -> None:
        ...
        for _ in range(n):
            ...
            if s is not None:
                prev_r = s[0]
                if wid[0] == self.foe and prev_r != r:
                    cands = self.intent_candidates(prev_r, r)
                    old = self.enemy_intent.get(wid)
                    policy_key = self.intent_policy_key(prev_r, r, cands)
                    base_edge_hot = (prev_r, r) in self.intent_hot_edges or (
                        policy_key is not None and policy_key in self.intent_policy)
                    if old is not None and self.turn - old[2] <= 2:
                        merged = old[0] & cands
                        steps = old[1] + 1
                        if not merged:
                            merged = cands
                        ...
                        steps = 1
                        old = None
                    target_key = self.target_policy_key(prev_r, r, cands, merged, steps)
                    target_hit = target_key is not None and target_key in self.target_policy
                    edge_hot = base_edge_hot or target_hit
                    group_trace += int(steps >= 2 and len(merged) <= 12 and self.my_hq in merged)
                    group_target += int(target_hit)
                    if old is not None:
                    ...
                    if target_hit:
                        steps = max(steps, 2)
                        hot_steps = max(hot_steps, 2)
                    self.enemy_intent[wid] = (merged, steps, self.turn, hot_steps)
                s[0] = r
                if s[2] and r == s[3]:
                    s[2] = False
        self._last_group_hq_turn = self.turn
        self._last_group_trace = group_trace
        self._last_group_target = group_target
        ...
        self.enemy_intent = {w: t for w, t in self.enemy_intent.items() if w in self.enemy}
