...
class Bot:
    ...
    def read_init(self) -> None:
        ...
        self.en_gold = 500.0  # 적 골드 상한 추정 (이동비 무시 = 과대추정)
    ...
    def plan(self) -> list[str]:
        ...
        # HP 레이스 솔버 -- 철회 (2026-07-06 밤). 적 골드 고갈 관측은 맞았지만
        # (무승부 8게임 전부 T195 적 골드 < 수리비) 이를 "즉시 총공격"으로
        # 연결하는 결정 로직이 실측에서 실패했다: 무검증판은 국지 전력
        # 확인 없이 몰빵해 병력을 헌납(t160->170 29->13 전멸, 순손해),
        # 단일 턴 전투 시뮬로 게이팅한 검증판은 반대로 포탑+전체HP를 한
        # 턴에 못 뚫어 사실상 항상 미발동(죽은 코드)이었다. 진짜 필요한
        # 건 여러 턴에 걸친 지속 공성 캠페인 모델인데, 오늘 밤 안에 제대로
        # 풀 문제가 아니라 판단해 트리거를 철회한다. en_gold 추적 인프라는
        # 관측 정확도가 검증됐으므로 남겨두고 다음 사이클에 재설계한다.
    ...
    def read_result(self) -> None:
        ...
        for _ in range(n):
            ...
            b = self.buildings.get(r)
            if b is None:
                self.en_gold -= BASE_LV[1][0]
                self.buildings[r] = [side, False, 1, BASE_LV[1][1]]
            elif b[1]:
                self.en_gold -= HQ_LV[b[2] + 1][0] if b[2] < 5 else HQ_REPAIR_COST
                if b[2] < 5:
                    b[2] += 1
                b[3] = HQ_LV[b[2]][2]
            else:
                self.en_gold -= BASE_LV[b[2] + 1][0] if b[2] < 3 else BASE_REPAIR_COST
                if b[2] < 3:
                    b[2] += 1
                b[3] = BASE_LV[b[2]][1]
        ...
        # 적 골드 정산: 수입(그들 병력 위치 x 건물 노동한도, 완전 관측) - 보급
        en_income = 0
        for r, b in self.buildings.items():
            if b[0] != self.foe:
                continue
            here = sum(1 for w in self.enemy.values() if w[0] == r)
            cap = HQ_LV[b[2]][5] if b[1] else BASE_LV[b[2]][3]
            en_income += WORK_INCOME * min(here, cap)
        self.en_gold = max(0.0, self.en_gold + en_income
                           - UPKEEP * len(self.enemy))
        self._eg_hist = getattr(self, "_eg_hist", [])
        self._eg_hist.append(self.en_gold)
