...
def valid_data_payload(data: object) -> bool:
    return (
        ...
        and data.get("source_bot") == os.path.basename(__file__)
    )
...
def decide(S: GameState, M: GameMap, P: Paths, turn: int) -> Actions:
    ...
    if losing_economy and base_count >= 2 and len(mine) >= 8 and plan_max_bases > 0:
        desired_bases = max(desired_bases, min(plan_max_bases, max(3, enemy_base_count - 2)))
    ...
    if map_hint and mode == "turtle_tech" and plan_max_bases > 0 and not urgent_home_threat and not any(w.region == M.my_hq for w in enemies):
        for turn_idx, target_req in ((1, 2), (2, 3), (3, 4), (4, 5)):
            turn_req = policy_value(map_hint, turn_idx, 999)
            army_req = policy_value(map_hint, turn_idx + 4, 0)
    ...
    for w in idle:
        if w.region in pending_build_regions:
    ...
    contest_sent = 0
    contest_home_sent = 0
    contest_excluded = set(builder_orders) | set(base_worker_orders) | set(base_defender_orders) | home_defender_orders | protected_workers
