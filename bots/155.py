...
DATA_FORMAT = "nypc-155-profile-json"
_DATA_CACHE = None
_DATA_LOADED = False
...
class Bot:
    def __init__(self, style: str) -> None:
        ...
        self.gold = 500
        self.enemy_intent: dict[str, tuple[frozenset[int], int, int]] = {}
        self._intent_cands: dict[tuple[int, int], frozenset[int]] = {}
        self.profile: dict = {}
        self.profile_plan: dict = {}
    ...
    def read_init(self) -> None:
        ...
        self.profile = self.resolve_profile()
        self.apply_profile()
        self.mp = dict(chip=True)  # 칩은 지고/비길 때만 발동하는 안전 규칙
        extra_mp = self.profile.get("mp")
        if isinstance(extra_mp, dict):
            for key in ("chip", "parity", "raid"):
                if isinstance(extra_mp.get(key), bool):
                    self.mp[key] = extra_mp[key]
        print("OK", flush=True)
    ...
    def load_data(self) -> dict:
        ...
        for path in self.data_bin_paths():
            ...
            try:
                ...
                continue
            if isinstance(data, dict) and data.get("format") == DATA_FORMAT:
                _DATA_CACHE = data
                _DATA_LOADED = True
                return data
        _DATA_CACHE = {}
    ...
    def _convert_region(self, value):
        if not isinstance(value, int):
            return value
        if not 0 <= value < self.N:
            return value
        return self._tr(value) if self.flip else value

    def _convert_plan(self, plan: dict) -> dict:
        out = {}
            ...
            if isinstance(value, list):
                ...
                for item in value:
                    if isinstance(item, int):
                        converted.append(self._convert_region(item))
                    elif isinstance(item, list):
                        converted.append([
                            self._convert_region(part) if idx == len(item) - 1 else part
                ...
                out[key] = self._convert_region(value)
            else:
                out[key] = value
        return out

    def apply_profile(self) -> None:
        p_overrides = self.profile.get("p")
        if isinstance(p_overrides, dict):
            for key, value in p_overrides.items():
                if key in self.p and isinstance(value, (int, float)):
                    self.p[key] = value
        raw_plan = self.profile.get("plan")
        if isinstance(raw_plan, dict):
            side_key = "right" if self.flip else "left"
            side_plan = raw_plan.get(side_key)
            if isinstance(side_plan, dict):
                self.profile_plan = self._convert_plan(side_plan)

    def profile_regions(self, key: str) -> list[int]:
        value = self.profile_plan.get(key)
        if not isinstance(value, list):
            return []
        return [r for r in value if isinstance(r, int) and 0 <= r < self.N]
    ...
    def hq_intent_count(self) -> int:
        if not getattr(self, "unknown_map", False):
            return 0
        count = 0
        for wid, (cands, steps, last_turn) in self.enemy_intent.items():
            if wid not in self.enemy or self.turn - last_turn > 1:
                continue
            if steps >= 2 and len(cands) <= 5 and self.my_hq in cands:
                count += 1
        return count
    ...
    def plan(self) -> list[str]:
        ...
        if delayed_mass_alert:
            rush_alert = True
        if incoming_hq >= 4 and self.turn < 80:
            rush_alert = True
        # 올인 러시 감지:
        ...
        under_pressure = raided or near_enemy >= 1 or outmassed or (
            incoming_hq >= 4 and self.turn < 120)
        ...
        if n_bases < base_target and expandable:
            prof_order = self.profile_regions("safe_expansion_order")
            if prof_order:
                build_order = [r for r in prof_order if r in self.strongholds]
                build_order += [r for r in self.strongholds if r not in set(build_order)]
            else:
        ...
        if getattr(self, "unknown_map", False):
            ...
            threatened = (
                ...
                or (incoming_hq >= 5 and self.turn < 120)
            )
        ...
        if expansion_ok and n_bases < base_target:
            ...
            if getattr(self, "unknown_map", False):
                prof_order = self.profile_regions("expansion_order")
                if prof_order:
                    rank = {r: i for i, r in enumerate(prof_order)}
                    targets.sort(key=lambda r: (rank.get(r, 10**6),
                                                self.dist[self.my_hq][r],
        ...
        if getattr(self, "unknown_map", False):
            planned_stage = self.profile_plan.get("stage_region")
            if isinstance(planned_stage, int) and planned_stage in my_regions:
                staging = planned_stage
            else:
                staging = min(
                    my_regions,
            ...
            if s is not None:
                ...
                if wid[0] == self.foe and prev_r != r:
                    ...
                    if old is not None and self.turn - old[2] <= 2:
                        ...
                        merged = cands
                        steps = 1
                    self.enemy_intent[wid] = (merged, steps, self.turn)
                s[0] = r
                if s[2] and r == s[3]:
                    s[2] = False
