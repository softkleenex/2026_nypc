...
class Bot:
    ...
    def plan(self) -> list[str]:
        ...
        if ((army_size >= p["push_army"] and hq_lv >= p["hq_push_lv"] and advantage)
            ...
            self.attacking = True
        elif self.attacking and alive < enemy_alive - 5:
            self.attacking = False  # 공세가 손해로 판명되면 재정비 (헌납 방지)
        ...
        if hq is not None:
            ...
            cap = HQ_LV[lv][4]
            # 병력 열세에서는 저축(테크)보다 훈련이 먼저다. 33이 7번에게 진
            # 원인: 적응형 상대가 자유 성장하는 동안 병력 10명에서 동결된 채
            # HQ 테크 저축만 했다 (30은 그림자 버그의 '낮은 댐' 덕에 우연히
            # 훈련이 흘렀고 31명으로 이겼다). 저축은 동수 이상일 때만.
            behind = len(self.mine) < len(self.enemy)
            ...
            # 군대가 융해된다 (서버 실측: 30.py가 4번전 굶주림 61회, 5번전
            # 183회로 전사 수십 명분을 잃음. 이 펀드 도입 후 31/32는 0회).
            supply_fund = 0
            if self.turn >= 160:
